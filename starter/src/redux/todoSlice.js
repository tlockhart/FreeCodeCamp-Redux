...
export const toggleCompleteAsync = createAsyncThunk(
  ...
  async (payload) => {
    ...
    if (response.ok) {
      /* whenever the thunk returns its going to dispatch 
    an action and it will return the content below */
      const todo = await response.json();
  ...
  extraReducers: {
    ...
    [toggleCompleteAsync.fulfilled]: (state, action) => {
      const index = state.findIndex(
        ...
        // set the completed value for the index in state.
        // redux will update the state and tell the components
        // that they need to rerender.
        state[index].completed = action.payload.completed;
    }
  },
});
